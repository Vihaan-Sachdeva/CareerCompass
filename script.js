...
function renderCareer(career) {
  careerCardEl.innerHTML = `
    <h3>${career.title}</h3>
    <p>Open the sections below to learn more about this career path.</p>
    ...
    ${createDetailSection("Overview", `<p>${career.overview}</p>`)}
    ${createDetailSection("What you'll do", createBulletList(career.responsibilities))}
    ${createDetailSection("Education pathway", `<p>${career.educationPath}</p>`)}
    ${createDetailSection("Work environment", `<p>${career.workEnvironment}</p>`)}
    ${createDetailSection("Core skills to build", createBulletList(career.keySkills))}
    ${createDetailSection("Student action plan", createBulletList(career.studentActionPlan))}
  `;
}
...
function showView(targetView) {
  ...
  navLinks.forEach((link) => {
    link.classList.toggle("active", link.dataset.viewTarget === targetView);
  });
